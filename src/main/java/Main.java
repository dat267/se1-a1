...
import a1_2001040024.Doc;
import a1_2001040024.Engine;
import a1_2001040024.Match;
import a1_2001040024.Query;
import a1_2001040024.Result;
import a1_2001040024.Word;
...
public class Main {
    public static void main(String[] args) {
        ...
        if (Word.stopWords.size() != 174) {
            System.out.println(
                    "Word.loadStopWords(): incorrect number of stopWords loaded (expected: 174)");
        }
        Engine e = new Engine();
        ...
        if (Word.createWord("").isKeyword())
            System.out.println(
                    "Word.createWord(): empty string ('') should be an invalid word (not a keyword)");
        if (Word.createWord("123456").isKeyword())
            System.out.println(
                    "Word.createWord(): '123456' should be an invalid word (not a keyword)");
        if (Word.createWord("!@#$%^").isKeyword())
            System.out.println(
                    "Word.createWord(): '!@#$%^' should be an invalid word (not a keyword)");
        if (Word.createWord("se2021").isKeyword())
            System.out.println(
                    "Word.createWord(): 'se2021' should be an invalid word (not a keyword)");
        if (Word.createWord(" and").isKeyword())
            System.out.println(
                    "Word.createWord(): ' and' should be treated as an invalid word (not a keyword because it contains a space)");
        if (Word.createWord(",se2021.").isKeyword())
        ...
        if (!Word.createWord(",se2021.").getText().equals(",se2021."))
            System.out
                    .println("Word.createWord(): the text part of ',se2021.' should be ',se2021.'");
        ...
        if (!Word.createWord("apple").equals(Word.createWord("Apple")))
            System.out.println(
                    "Word.equals() should be case-insensitive, so 'apple' should be equal to 'Apple'");
        if (!Word.createWord("content").equals(Word.createWord("\"content\".")))
            System.out.println(
                    "Word.equals() should compare the text part only, so 'content' should be equal to '\"content\".'");
        ...
        wordTexts[1] = new String[] {"Definition", "An", "object-oriented", "system", "context",
        ...
        wordTypes[1] =
                new boolean[] {true, false, true, true, true, true, false, false, true, true};
            ...
            if (tmp.size() != listSizes[x]) {
                ...
                for (int i = 0; i < tmp.size(); i++) {
                    if (!wtxt[i].equals(tmp.get(i).getText()))
                        System.out.println(methods[x] + ": incorrect word text '"
                                + tmp.get(i).getText() + "' (expected '" + wtxt[i] + "')");
                    if (!wpf[i].equals(tmp.get(i).getPrefix()))
                        System.out.println(methods[x] + ": incorrect word prefix '"
                                + tmp.get(i).getPrefix() + "' (expected '" + wpf[i] + "')");
                    if (!wsf[i].equals(tmp.get(i).getSuffix()))
                        System.out.println(methods[x] + ": incorrect word suffix '"
                                + tmp.get(i).getSuffix() + "' (expected '" + wsf[i] + "')");
                    if (wtp[i] != tmp.get(i).isKeyword())
                        System.out.println(methods[x] + ": incorrect isKeyword for '"
                                + tmp.get(i).toString() + "' (expected: " + wtp[i] + ")");
                }
            }
        }
        // Testing Query & Match
        ...
        for (int i = 0; i < matches.size(); i++) {
            if (!matches.get(i).getWord().getText().equals(matchedWords[i])) {
                System.out.println("Query.matchAgainst(): incorrect word '"
                        + matches.get(i).getWord().getText() + " (expected: '" + matchedWords[i]
                        + "')");
            }
        }
        // Testing Result
        ...
        for (int i = 0; i < results.size(); i++) {
            ...
            if (x != matchCounts[i]) {
                System.out.println("Engine.search(): incorrect match count (actual: " + x
                        + ", expected: " + matchCounts[i]);
            }
        }
        String tmpTitle = results.get(0).getDoc().getTitle().toString();
        if (!tmpTitle.equals("[System, context, and, interactions]")) {
            System.out.println("Engine.search(): incorrect first result '" + tmpTitle
                    + "' (expected '[System, context, and, interactions]'");
        }
        tmpTitle = results.get(1).getDoc().getTitle().toString();
        if (!tmpTitle.equals("[Design, patterns]")) {
            System.out.println("Engine.search(): incorrect second result '" + tmpTitle
                    + "' (expected '[Design, patterns]'");
        }
        // Testing HTML highlights
        try (BufferedReader bufferedReader = new BufferedReader(new FileReader("testCases.html"))) {
