...
/**
 * The Word class represents a word from a document.
 * A Word object consists of a prefix, text part, a suffix, and a boolean flag to indicate its validity.
 * The class also includes a static set of stop words.
 */
public class Word {
	...
	public static Word createWord(String rawText) {
		...
		String text = rawText.replaceAll("[^\\p{L}'-]", "");
		// Split rawText into two parts using text in the middle
		String[] parts = rawText.split(text, 2);
		// In case split has no match, it returns a 1 element array with the original rawText, thus making rawText assigned to prefix
		String prefix = parts[0];
		// If parts has two elements, assign the second element to suffix, otherwise set empty string as suffix
		String suffix = parts.length == 2 ? parts[1] : "";
		// Word is valid if
		boolean valid = !text.isEmpty() // Text part is not empty
		...
		// If word is invalid, reassign attributes appropriately
		if (!valid) {
			prefix = "";
			text = rawText;
			suffix = "";
		}
		// Use 's suffix for texts with 's ending
		if (text.endsWith("'s")) {
	...
	/**
	 * Computes the hash code for this Word instance.
	 * The hash code is computed based on the text part of the Word, case-insensitively.
	 * This means that two Word instances with the same text part but different cases will have the same hash code.
	 * The hash code is computed by calling the hashCode() method on the lowercase text part of the Word.
	 *
